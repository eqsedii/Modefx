...
// PayHero calls this when the STK payment finishes. This is the ONLY place a plan is activated.
export default async function handler(req, res) {
  ...
  if (success && amountOk) {
    // Conditional update: only the first delivery wins, so a repeated callback can't double-activate.
    ...
    if (won && won.length) {
      const plan = PLANS[pay.plan_id];
      await admin.rpc("activate_subscription", { p_user: pay.user_id, p_plan: pay.plan_id, p_days: plan.days, p_payment: pay.id });
    }
  } else {
