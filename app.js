(() => {
  ...
  document.addEventListener("click", (e) => {
    ...
    if (b.hasAttribute("data-async") || b.hasAttribute("data-quiet")) return; // handled elsewhere
    Loader.flash();
  });

  /* ---------------- Intro (once per session, before the landing page) ---------------- */
  (function intro() {
    const el = $("#intro"), v = $("video", el);
  ...
  if (C.brokerUrl) $("#broker-link").href = C.brokerUrl;

  if (C.supportEmail || C.supportWhatsApp) {
    $("#support").hidden = false; $("#nav-support").hidden = false;
  ...
  // Demo markets: seeded so the sparklines are stable. Not live data.
  const DEMO = [
  ...
  $("#market-list").innerHTML = DEMO.map((m) => {
    const up = m.chg >= 0;
  ...
  const notConnected = (el) => msg(el, "Sign-in isn't connected yet. Add your Supabase URL and anon key in config.js.", "info");
  ...
  function goToAuth(m) {
    ...
    setTimeout(() => { if (!$("#auth-forms").hidden) $("#email").focus({ preventScroll: true }); }, 500);
  }
  $$("[data-goto-auth]").forEach((b) => b.addEventListener("click", () => goToAuth()));
  ...
  $("#auth-form").addEventListener("submit", async (e) => {
    ...
    try {
      ...
      if (!client) { notConnected(authMsg); return; }
  ...
  $("#google").addEventListener("click", async () => {
    const client = await getSB();
    if (!client) return notConnected(authMsg);
    const { error } = await Loader.run(client.auth.signInWithOAuth({
  ...
  $("#forgot").addEventListener("click", async () => {
    ...
    const client = await getSB();
    if (!client) return notConnected(authMsg);
    const { error } = await Loader.run(client.auth.resetPasswordForEmail(email, { redirectTo: location.origin + location.pathname }));
    if (error) return msg(authMsg, friendly(error));
  ...
  async function render(s) {
    session = s;
    const signedIn = !!s;
    $("#auth-forms").hidden = signedIn || !$("#recovery-form").hidden;
    $("#account").hidden = !signedIn || !$("#recovery-form").hidden;
    $("#nav-signin").textContent = signedIn ? "Account" : "Sign in";
    if (signedIn) {
  ...
  const dlg = $("#checkout"), coMsg = $("#co-msg"), coPay = $("#co-pay");
  ...
  planList.addEventListener("click", (e) => {
    ...
    if (!session) { goToAuth("signup"); msg(authMsg, `Create an account or sign in to get ${chosen.name}.`, "info"); return; }
  ...
  $("#co-form").addEventListener("submit", async (e) => {
    ...
    try {
      ...
      $("#co-form").hidden = false; coPay.hidden = false; coPay.textContent = "Waiting for confirmation…";
  ...
  function poll(ref, started) {
    pollTimer = setTimeout(async () => {
      ...
      if (data && data.status === "failed") {
        msg(coMsg, "The payment wasn't completed" + (data.failure_reason ? ` (${data.failure_reason}).` : ".") + " You have not been charged unless M-Pesa says otherwise. Try again.");
